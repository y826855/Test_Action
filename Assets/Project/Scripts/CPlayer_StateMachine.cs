...
public class CPlayer_StateMachine : MonoBehaviour
{
    //상태 : 대기, 숙이기, 점프, 대쉬 등등
    ...
    private void Awake()
    {
        ...
        m_AnimFunc.m_RootMotion = MoveRootMotion;
    }
    ...
    {
        m_CurrMoveState = EMoveState.TURN;
    ...
    public void StartComboCheck() 
    {
        ...
        m_AnimCtrl.SetLayerWeight(2, 1);
    }
    public void EndComboCheck() 
    {
        Debug.Log("END COMBO");
        ActionReset();
        StartCoroutine(CoAnimWeight());
    }

    IEnumerator CoAnimWeight() 
    {
        float t = 1;
        while (t >= 0) 
        {
            t -= Time.deltaTime;
            m_AnimCtrl.SetLayerWeight(2, t);
            yield return null;
        }
    }
    ...
    void MoveRootMotion() 
    {
        m_CharController.Move(m_AnimCtrl.deltaPosition);
        m_Body.rotation *= m_AnimCtrl.deltaRotation;

        //m_BodyRot_Dest += m_AnimCtrl.deltaRotation.eulerAngles;
    }
